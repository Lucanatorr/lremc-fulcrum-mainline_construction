...
| OI-02 | — | `forms_update` refuses all writes, including a call with no changes. Auth, role (Owner/can_manage_apps) and payload validity all ruled out | The unit fix and the timezone fix cannot be deployed; records entered today save with a null unit and, pre-fix, a wrong work date | Deploy is a replay of validated payloads once writes return | **P1** | Retry; precedent is it clears on its own (3 days last time) | OPEN |
